...
The most basic element of the AST is a =Literal=. These are either
integer, float, or string literals.
...
We also want to add a couple =From= impls so creating a =LiteralKind= is
easy to do.
