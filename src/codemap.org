* The CodeMap
  ...
  :END:
A =CodeMap= gives you a central mapping from spans to their location in
the group of files being analysed.
...
We start off with a =Span=. This is really just a wrapper around an
integer, with the assumption that a span will *always* correspond to
something in the =CodeMap=. This means using a span from one =CodeMap=
with another will result in a panic if you are lucky, or silently give
you garbage.
...
For our purposes, the =CodeMap= will just contain a list of =FileMap=s.
These keep track of their name, contents, and the mapping of spans to
locations in that content.
...
The codemap has a couple useful methods for adding new files and looking
up the string corresponding to a span.
...
You may have noticed that =FileMap= contains a =RefCell<HashMap<_>>=.
This is because we want to pass around multiple pointers to a file
mapping, yet still be able to add new spans if we want to. It also
contains a reference to the parent =CodeMap='s counter so when we insert
new spans into the =FileMap= they'll still get globally unique IDs.
...
Users can freely add new spans to a =FileMap=, to do this we'll take in
the start and end indices, create a new span ID by incrementing our
counter, then we insert the new span and range into the =items=. In
debug builds we'll do bounds checks, but it's an assumption that the
=start= and =end= indices are both within bounds, and lie on valid
...
To help after the tokenizing step, lets add a method which will take a
bunch of tokens and register them with a =FileMap=. The same caveats as
with =insert_span()= will apply here.
...
To test that our =CodeMap= and =FileMap= behave as we expect them to,
let's create some dummy "files" and try to create spans in them.
